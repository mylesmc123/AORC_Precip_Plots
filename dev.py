...
plt.colorbar()
# add title
plt.title(f'{events[0]} Cumulative Precipitation')
...
for i in range(data['APCP_surface'].shape[0]):
    ...
    thisData = data['APCP_surface'][i].values
    # r = rstats.zonal_stats(domain, data['APCP_surface'][i], affine=profile['transform'], stats='mean')
    r.appendrstats.zonal_stats(domain, thisData, affine=profile['transform'], stats='mean')
